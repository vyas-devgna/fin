...
/**
 * Invokes system UPI Intent to open installed payment apps (GPay, PhonePe, Paytm, BHIM).
 * If running inside native Android WebView, delegates directly to Kotlin FinNative bridge.
 */
export async function launchUpiIntent(uriOrConfig) {
  const uri = typeof uriOrConfig === 'string' ? uriOrConfig : buildUpiUri(uriOrConfig);
  const parsed = parseUpiUri(uri);
  if (!parsed.valid) throw new Error(parsed.error);

  if (window.FinNative && typeof window.FinNative.launchUpiIntent === 'function') {
    return window.FinNative.launchUpiIntent(uri);
  } else if (navigator.userAgent.match(/Android|iPhone|iPad|iPod/i)) {
    // On mobile web/PWA, navigating to the custom URI scheme prompts the OS app selector
    window.location.href = uri;
    return true;
  } else {
    // Desktop simulation fallback
    console.info('Simulated UPI Intent Launch:', parsed);
    return false;
  }
}

/**
 * Initiates QR scanning via Android camera bridge or HTML5 BarcodeDetector fallback.
 */
export function scanUpiQr(onResult, onError) {
  if (window.FinNative && typeof window.FinNative.startQrScanner === 'function') {
    // Register global receiver for native bridge callback
    window.FinApp = window.FinApp || {};
    window.FinApp.onQrScanned = (qrString) => {
      const parsed = parseUpiUri(qrString);
      onResult(parsed);
    };
    window.FinApp.onQrScanError = (errMsg) => {
      if (onError) onError(errMsg);
    };
    window.FinNative.startQrScanner();
  } else if ('BarcodeDetector' in window) {
    // HTML5 camera fallback for Chromium/Android browsers
        ...
        const interval = setInterval(async () => {
          try {
            ...
            if (barcodes.length > 0) {
              ...
              onResult(parseUpiUri(barcodes[0].rawValue));
            }
          } catch (e) {
            // IGNORE frame decoding errors during stream
          }
        }, 300);
      })
      .catch((err) => {
        if (onError) onError('Camera permission denied or unavailable');
      });
  } else {
    if (onError) onError('Native scanner and BarcodeDetector unavailable. Please paste UPI QR code or VPA link.');
  }
}
